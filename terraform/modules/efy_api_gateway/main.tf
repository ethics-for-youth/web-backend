...
resource "aws_api_gateway_method" "dua_put" {
  rest_api_id   = aws_api_gateway_rest_api.efy_api.id
  resource_id   = aws_api_gateway_resource.duas.id
  http_method   = "PATCH"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "dua_put" {
  rest_api_id             = aws_api_gateway_rest_api.efy_api.id
  resource_id             = aws_api_gateway_resource.duas.id
  http_method             = aws_api_gateway_method.dua_put.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.dua_put_lambda_arn
}

resource "aws_lambda_permission" "dua_put" {
  statement_id  = "AllowExecutionFromAPIGateway-dua-put"
  action        = "lambda:InvokeFunction"
  function_name = var.dua_put_lambda_function_name
  principal     = "apigateway.amazonaws.com"
...
resource "aws_api_gateway_resource" "duas_id" {
  rest_api_id = aws_api_gateway_rest_api.efy_api.id
  parent_id   = aws_api_gateway_resource.duas.id
  path_part   = "{id}"
}

resource "aws_api_gateway_method" "dua_delete" {
  rest_api_id   = aws_api_gateway_rest_api.efy_api.id
  resource_id   = aws_api_gateway_resource.duas_id.id
  http_method   = "DELETE"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "dua_delete" {
  rest_api_id             = aws_api_gateway_rest_api.efy_api.id
  resource_id             = aws_api_gateway_resource.duas_id.id
  http_method             = aws_api_gateway_method.dua_delete.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.dua_delete_lambda_arn
}

resource "aws_lambda_permission" "dua_delete" {
  statement_id  = "AllowExecutionFromAPIGateway-dua-delete"
  action        = "lambda:InvokeFunction"
  function_name = var.dua_delete_lambda_function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.efy_api.execution_arn}/*/DELETE/duas/*"
}
